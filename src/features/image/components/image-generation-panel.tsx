...
export function ImageGenerationPanel() {
  ...
  const [latestDescription, setLatestDescription] = useState<string | null>(null);
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      if (payload.history[0]) {
        setLatestImageUrl(payload.history[0].imageUrl);
        setLatestDescription(payload.history[0].output);
      }
    } catch {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setPrompt("");
      setUsage(payload.usage);
      setLatestImageUrl(payload.generation.imageUrl);
      setLatestDescription(payload.generation.output);
      await loadHistory();
    } catch {
      setErrorMessage("Unable to reach the server. Please try again.");
    } finally {
  ...
  return (
    <section className="space-y-6">
      <header>
        <h2 className="text-2xl font-semibold tracking-tight text-slate-100">Image Generation</h2>
        <p className="mt-1 text-sm text-slate-400">
          Generate image concepts with a wider layout and a dedicated insights rail.
        </p>
      </header>
        ...
        <div className="space-y-4">
          <article className="rounded-xl border border-[#2d313a] bg-[#171b25] p-6">
            ...
            <textarea
              ...
              placeholder="A futuristic electric sports car at sunset, cinematic lighting, ultra detailed."
            />
            <p className="mt-1 text-xs text-slate-500">{prompt.length} / 2000</p>
          ...
          <article className="rounded-xl border border-[#2d313a] bg-[#171b25] p-6">
            ...
            {latestImageUrl ? (
              <div className="mt-4 space-y-4">
                <Image
                  src={latestImageUrl}
                  alt={latestDescription || "Generated image"}
                  width={1200}
                  height={680}
                  className="h-80 w-full rounded-lg border border-[#2d313a] object-cover"
                />
                <p className="text-sm leading-6 text-slate-300">{latestDescription ?? "Generated image"}</p>
              </div>
            ) : (
              <p className="mt-4 text-sm text-slate-500">No output yet. Generate an image to see results.</p>
            )}
          </article>
          ...
          <article className="overflow-hidden rounded-xl border border-[#2d313a] bg-[#171b25]">
            <header className="flex items-center justify-between border-b border-[#2d313a] px-6 py-4">
              <h3 className="text-base font-semibold text-slate-100">Recent Image Generations</h3>
              <button
                type="button"
                onClick={() => {
                  void loadHistory();
            ...
            {isLoadingHistory ? (
              ...
              <p className="px-6 py-5 text-sm text-slate-500">No image generations yet.</p>
            ) : (
                      ...
                      <tr key={entry.id}>
                        <td className="px-6 py-4">
                          {entry.imageUrl ? (
                            <Image
                              src={entry.imageUrl}
                              alt={entry.output || "Generated image"}
                              width={140}
                              height={84}
                              className="h-16 w-28 rounded-md border border-[#2d313a] object-cover"
                            />
                          ) : (
                            <div className="flex h-16 w-28 items-center justify-center rounded-md border border-[#2d313a] text-xs text-slate-500">
                              No image
                            </div>
                          )}
                        </td>
                        <td className="max-w-xl px-6 py-4">
                          ...
                          <p className="mt-1 line-clamp-1 text-xs text-slate-400">{entry.output}</p>
                        </td>
