...
export function CodeGenerationPanel() {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/ai/code?limit=${DEFAULT_HISTORY_LIMIT}`, {
        method: "GET",
      });
  ...
  return (
    <section className="space-y-6">
      <header>
        <h2 className="text-2xl font-semibold tracking-tight text-slate-100">Code Generation</h2>
        <p className="mt-1 text-sm text-slate-400">
          Generate implementation-ready code with a wider development workspace.
        </p>
      </header>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-4">
          <article className="rounded-xl border border-[#2d313a] bg-[#171b25] p-6">
            <label className="mb-2 block text-sm font-medium text-slate-300" htmlFor="code-prompt">
              Prompt
            </label>
            <textarea
              id="code-prompt"
              ...
              placeholder="Build a typed Next.js API route that validates payloads with Zod and logs safe error metadata."
            />
          ...
          <article className="rounded-xl border border-[#2d313a] bg-[#171b25] p-6">
            ...
            {result ? (
              <pre className="mt-4 max-h-[26rem] overflow-auto whitespace-pre-wrap rounded-lg border border-[#2d313a] bg-[#0f1115] p-4 text-sm leading-6 text-slate-200">
                {result}
              </pre>
            ) : (
              <p className="mt-4 text-sm text-slate-500">No output yet. Generate code to see results.</p>
            )}
          </article>
          ...
          <article className="overflow-hidden rounded-xl border border-[#2d313a] bg-[#171b25]">
            <header className="flex items-center justify-between border-b border-[#2d313a] px-6 py-4">
              <h3 className="text-base font-semibold text-slate-100">Recent Code Generations</h3>
              <button
                type="button"
                onClick={() => {
                  void loadHistory();
            ...
            {isLoadingHistory ? (
              ...
              <p className="px-6 py-5 text-sm text-slate-500">No code generations yet.</p>
            ) : (
                    ...
                    {sortedHistory.map((entry) => (
                      <tr key={entry.id}>
                        <td className="max-w-xl px-6 py-4">
                          ...
                          <pre className="mt-1 line-clamp-3 whitespace-pre-wrap text-xs text-slate-400">
                            {entry.output}
                          </pre>
                        </td>
