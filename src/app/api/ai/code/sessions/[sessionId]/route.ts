...
import {
  deleteCodeSessionForUser,
  updateCodeSessionTitleForUser,
...
export async function PATCH(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const rateLimit = enforceRateLimit({
    key: `ai:code:sessions:update:${user.id}:${getRequestIp(request)}`,
    limit: 30,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    const session = await updateCodeSessionTitleForUser(user.id, sessionId, input.title);
...
export async function DELETE(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const rateLimit = enforceRateLimit({
    key: `ai:code:sessions:delete:${user.id}:${getRequestIp(request)}`,
    limit: 20,
    windowMs: 60_000,
  });
  if (!rateLimit.allowed) {
  ...
  try {
    const { sessionId } = sessionParamsSchema.parse(await context.params);
    await deleteCodeSessionForUser(user.id, sessionId);
