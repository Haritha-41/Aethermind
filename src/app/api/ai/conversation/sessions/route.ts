...
import {
  createConversationSessionForUser,
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const sessions = await listConversationSessionsForUser(user.id, query.limit);
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const rateLimit = enforceRateLimit({
    key: `ai:conversation:sessions:${user.id}:${getRequestIp(request)}`,
    limit: 20,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    const session = await createConversationSessionForUser(user.id, input.title);
