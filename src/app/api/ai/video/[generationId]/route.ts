...
import { deleteVideoGenerationForUser } from "@/server/services/ai/video-service";
...
export async function DELETE(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const rateLimit = enforceRateLimit({
    key: `ai:video:delete:${user.id}:${getRequestIp(request)}`,
    limit: 25,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    await deleteVideoGenerationForUser(user.id, generationId);
