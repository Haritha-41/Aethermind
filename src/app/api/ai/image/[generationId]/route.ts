...
import { deleteImageGenerationForUser } from "@/server/services/ai/image-service";
...
export async function DELETE(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const rateLimit = enforceRateLimit({
    key: `ai:image:delete:${user.id}:${getRequestIp(request)}`,
    limit: 25,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    await deleteImageGenerationForUser(user.id, generationId);
